...
from zoo.board_games.othello.envs.get_done_winner_cython import get_done_winner_cython
from zoo.board_games.othello.envs.legal_actions_cython import legal_actions_cython

from zoo.board_games.alphabeta_pruning_bot import AlphaBetaPruningBot


@lru_cache(maxsize=512)
def _legal_actions_func_lru(board_tuple, current_player):
    ...
    return legal_actions_cython(board_view, current_player)
...
@ENV_REGISTRY.register('othello')
class OthelloEnv(BaseEnv):
    ...
    def legal_actions(self):
        ...
        return _legal_actions_func_lru(tuple(map(tuple, self.board)), self.current_player)

    # only for evaluation speed
    @property
    def legal_actions_cython(self):
        return legal_actions_cython(list(self.board), self.current_player)
    ...
    def reset(self, start_player_index=0, init_state=None, katago_policy_init=False, katago_game_state=None):
        ...
        if init_state is not None:
            ...
            # Initial state for othello
            for i in range(3, 5):
                for j in range(3, 5):
                    self.board[i, j] = (i + j) % 2 + 1
    ...
    def step(self, action):
        if self.battle_mode == 'self_play_mode':
            if self.prob_random_agent > 0:
                ...
                if np.random.rand() < self.prob_expert_agent: # [gskim] check needed
    ...
    def _player_step(self, action):
        ...
        if done:
            ...
            # print('Othello one episode done: ', info)
        action_mask = np.zeros(self.total_num_actions, 'int8')
        action_mask[self.legal_actions] = 1
        obs = {
            'observation': self.current_state()[1],
    ...
    def rule_bot_v0(self):
        # """
        # Overview:
        #     Hard coded expert agent for Othello env.
    ...
    def simulate_action(self, action): # [gskim] ckpt 0812
        """
        Overview:
        ...
        for dx, dy in directions:
            ...
            if 0 <= x < 8 and 0 <= y < 8 and new_board[x, y] == self.current_player:
                for fx, fy in pieces_to_flip:
                    new_board[fx, fy] = self.current_player

        # Switch players
        next_simulator_env = copy.deepcopy(self)
        next_simulator_env.reset(1 - self.start_player_index, init_state=new_board)
    ...
    def render(self, mode="human"):
        ...
        if mode == 'human':
            ...
            plt.title('Othello: ' + ('Black Turn' if self.current_player == 1 else 'White Turn'))
